...
const deepEqual = import_lite.dequal;
...
const createForm = (config) => {
  ...
  const form = (0, import_index3.w)(getInitial.values());
  const errors = (0, import_index3.w)(getInitial.errors());
  const touched = (0, import_index3.w)(getInitial.touched());
  const isSubmitting = (0, import_index3.w)(false);
  const isValidating = (0, import_index3.w)(false);
  const isValid = (0, import_index3.d)(errors, ($errors) => {
    const noErrors = util.getValues($errors).every((field) => field === NO_ERROR);
    return noErrors;
  });
  const modified = (0, import_index3.d)(form, ($form) => {
  ...
  const isModified = (0, import_index3.d)(modified, ($modified) => {
  ...
  function validateFieldValue(field, value) {
    ...
    if (validationSchema) {
      ...
      return validationSchema.validateAt(field, (0, import_index.j)(form)).then(() => util.update(errors, field, "")).catch((error) => util.update(errors, field, error.message)).finally(() => {
