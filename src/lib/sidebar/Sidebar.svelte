...
<MediaQuery query="(max-width: 991px)" let:matches>
  {#if matches}
    <div
      in:fade
      class="d-flex justify-sb align-center b-radius-8 box_shadow-medium mobile-header"
    >
      <div
    ...
    <div class="menu">
      <nav>
        <div class="menu__items">
          <ul>
            <li
              class="menu__item--wrapper {current === 'overview'
                ? 'active'
                : ''}"
            ...
            <li
              class="menu__item--wrapper {current === 'profile'
                ? 'active'
                : ''}"
